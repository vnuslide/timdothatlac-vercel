...
const CFG = {
    ...
    BASE_TOKEN: process.env.LARK_APP_TOKEN,
...
function formatDateYMD_(d) {
    ...
    const parts = new Intl.DateTimeFormat('en-CA', options).formatToParts(date); // 'en-CA' cho yyyy-MM-dd
...
const SENSITIVE_PLACEHOLDERS = { /* ... (Giữ nguyên như trong file GAS của bạn) ... */ };

// Hàm chuyển đổi cho Supabase
function publicRecordForSupabase_(rec) {
  const f = rec.fields || {};
  const timeStr = f.ThoiGian ? formatDateYMD_(new Date(f.ThoiGian)) : '';
  ...
  return {
    ...
    time: timeStr, timeRaw: timeRaw, name,
    "group": group, description: description, docType: docType,
    khuVuc: khuVuc, image: originalImage, type: type,
    isPinned: f.Ghim === true,
    latitude: f.Latitude || null, longitude: f.Longitude || null,
    _name: normalizeText_(name), _group: normalizeText_(group),
    _docType: normalizeText_(docType), _khuVuc: normalizeText_(khuVuc),
...
// (Hàm chuyển đổi cho Gist - recordToPublic_() - Giữ nguyên như file GAS)
// ...
...
export default async function handler(request, response) {
    ...
    try {
        ...
        // 2. CHUẨN BỊ DỮ LIỆU ĐỂ SYNC
        // (Chúng ta sync TẤT CẢ, bao gồm "Chờ duyệt" để Trang Quản lý tin hoạt động)
        ...
        // const gistData = larkData.filter(it => (it.fields.TrangThai || 'Chờ duyệt') === 'Đã duyệt');
